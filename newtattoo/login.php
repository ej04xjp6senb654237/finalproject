			...
			<div id="contents">
				<div id="adbox">
					<div id="search">
						...
						<form action="login.php" method="post" enctype="multipart/form-data"><p>
							<ul>
								<li>
                                帳號：<input name="currUser" type="text" value="0000000000" size="15" maxlength="20" id="color">
								</li>
								<li>
                                密碼：<input name="currPwd" type="password"  value="tattoo" size="15" maxlength="20" id="brand">
								</li>
							</ul>								
				...
				<div id="main">
					<!-- 只要連到登入網頁，就要顯示所有商店 -->
						...
						//****提示**** 以下『兩條』指令要改	
						//****提示**** 查詢所有商店
						$SQLStr = " select * from 商品 where 商品代號 < 5 ";
						$rs = db_query($SQLStr); 
						if (mysql_num_rows($rs)>0) { 
								...
								for ($i=0; $i<$total; $i++)      {
									...
									echo  '<img src="images/'.$row["商品代號"] .'.jpg "alt="Img"> <br><span>'. $row['商品名稱'] .'</span>'.'</a>';
									echo "</li>";
									if (($i % 4)==3) {
										echo '</ul>';
									}
